...
\begin{document}
	...
	\end{figure}		
		...
		\begin{vibration_case_studies}
			Discuss the Sayano Shushenskaya Dam Disaster.
			\begin{figure}[H]
				\centering
				\includegraphics[width=4in]{../figures/Sayano-Shushenskaya_HPS_-_generator_hall}
				\caption{ Sayano Shushenskaya's turbine hall before the accident where the turbine that failed is in the very foreground of the image. \protect\footnotemark[1]}
			\end{figure}
			...
			\footnotetext[1]{4044415 ???????:  ?????? ?????? English:  Andrey Korzun, CC BY-SA 3.0 $<$https://creativecommons.org/licenses/by-sa/3.0$>$, via Wikimedia Commons}
