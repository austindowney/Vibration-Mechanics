...
% This example may not be needed. Check and replace it with a better one. 
%			\begin{example}
...
%				The system above can be described the EOM in standard form:
%				\begin{equation}
...
\end{example}
		...
		Sadly, engineers understanding of vortex shedding and structural dynamics lagged behind the development of these structures; leading to multiple wind-induced collapses of smokestacks during the industrial revolution.  The use of the transfer function approach gives the practitioner the ability to easily model the complex response of smokestack excited with a wide bandwidth of excitation. 
			...
			\end{figure}
			\footnotetext[2]{tromBer, CC BY-SA 3.0 $<$https://creativecommons.org/licenses/by-sa/3.0$>$, via Wikimedia Commons}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{../figures/Response_to_random_input_inputs}
...
\end{equation}
The subscript $xx$ denotes that this is a measure of the response for the variable $xx$, $\tau$ is the time difference between the values at which the signal $x(t)$ is sampled. The autocorrelation for the two inputs considered above is expressed in figure~\ref{fig:Response_to_random_input_autocorrelation}.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{../figures/Response_to_random_input_PSD}
