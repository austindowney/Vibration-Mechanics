...
\subsection{Harmonic Excitation of Undamped Systems}
 ...
 \begin{equation}
  \ddot{x}+\omega_\text{n}^2x = f_0\text{cos}(\omega t)
 \end{equation}     
 where:
 \begin{equation}
  f_0 = \frac{F_0}{m}
 ...
 \begin{equation}
  x_\text{h} = A\text{sin}(\omega_\text{n} t + \phi)
 \end{equation}   
 Next, we will denote the particular solution as $x_\text{p}$. $x_\text{p}$ can be determined by assuming that it is in the form of the forcing function, therefore:
 \begin{equation}
  f_0\text{cos}(\omega t)
 \end{equation} 
 becomes:
 \begin{equation}
  x_\text{p}  =X\text{cos}(\omega t)
 \end{equation}      
 where, $x_\text{p}$ is the particular solution and $X$ is the amplitude of the forced response. Our total solution for the harmonic excitation of undamped systems now becomes:
 \begin{equation}
  x(t) = A\text{sin}(\omega_\text{n} t + \phi) + X\text{cos}(\omega t) 
 \end{equation}    
 This approach, of assuming that $x_\text{p}=X\text{cos}(\omega t)$, in order to determine the particular solution, is called the \emph{method of undetermined coefficients}. To calculate $X$, first we take the equations for $x_\text{p}$ and $\ddot{x}_\text{p} $:
 \begin{equation}
  x_\text{p} = X\text{cos}(\omega t)
 \end{equation} 
 \begin{equation}
  \ddot{x}_\text{p} = -\omega^2X\text{cos}(\omega t)
 \end{equation}    
 and substituting these into the equation of motion in standard form yields:
 \begin{equation}
  -\omega^2X\text{cos}(\omega t)+\omega_\text{n}^2X\text{cos}(\omega t) = f_0\text{cos}(\omega t)
 \end{equation}  
 As long as  $\text{cos}(\omega t) \neq  0$, solving for X yields:
 \begin{equation}
  X = \frac{f_0}{\omega_\text{n}^2-\omega^2}
  \label{eq:X}
 \end{equation}  
 Therefore, as long as $\omega_\text{n} \neq \omega$, the particular solution can take the form:
 \begin{equation}
  x_\text{p} = \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{cos}(\omega t)
 \end{equation}      
 This then expands to the total form:
 \begin{equation}
  x(t) = A\text{sin}(\omega_\text{n} t + \phi) + \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{cos}(\omega t)
 \end{equation}    
 Expanding this to the general form for the homogeneous solution obtains the equation:
 \begin{equation}
  x(t) = A_1\text{sin}(\omega_\text{n} t) + A_2\text{cos}(\omega_\text{n} t) + \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{cos}(\omega t)
 \end{equation}    
 ...
 \begin{equation}
  x(0) = x_0 = A_2 + \frac{f_0}{\omega_\text{n}^2-\omega^2}
 \end{equation}    
 or:
 \begin{equation}
  A_2 = x_0-\frac{f_0}{\omega_\text{n}^2-\omega^2}
 \end{equation} 
 again, solving the equation in terms of velocity:
 \begin{equation}
  \dot{x}(t) = A_1\omega_\text{n}\text{cos}(\omega_\text{n} t) - A_2 \omega_\text{n} \text{sin}(\omega_\text{n} t) - \omega \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{sin}(\omega t)
 \end{equation} 
 and solving for the initial velocity at $t=0$:
 \begin{equation}
  \dot{x}(0) = v_0 =  A_1 \omega_\text{n}
 \end{equation}    
 or:
 \begin{equation}
  A_1 = \frac{v_0}{\omega_\text{n}}
 \end{equation}    
 Therefore, combining the equations, we get:
 \begin{equation}
  x(t) = \Big(\frac{v_0}{\omega_\text{n}}\Big)\text{sin}(\omega_\text{n} t) + \Big(x_0-\frac{f_0}{\omega_\text{n}^2-\omega^2}\Big)\text{cos}(\omega_\text{n} t) + \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{cos}(\omega t)
 \end{equation} 
 ...
 \end{equation}    
 \begin{equation}
  A = \sqrt{\bigg(\frac{v_0}{\omega_\text{n}}\bigg)^2+(x_0-X)^2}
 \end{equation}    
 \begin{equation}
  \phi = \text{tan}^{-1}\bigg(\frac{\omega_\text{n}(x_0-X)}{v_0}\bigg)
 \end{equation}    
 \begin{equation}
  X = \frac{f_0}{\omega_\text{n}^2-\omega^2}
 \end{equation}    
 ...
 \textbf{Visualizing Free and Forced Vibrations}
 ...
 \noindent For the 1-DOF system:
  ...
  \end{figure}
  \noindent with $k$ = 10~N/m, $m$ = 2.5~kg, $\omega$ = 4~rad/s, $F_0$ = 0.1~N, $x_0$ = 1~mm, and $v_0$ = 0~mm/s, plot the temporal responses of the system considering the free-vibration case and the excited case. Plot these on a single plot to compare the responses. 
...
\subsection{Harmonic Resonance}   
 ...
 Recall that our solution from before assumed that $\omega_\text{n} \neq \omega$; however, if $\omega_\text{n} = \omega$ then the system will develop the phenomenon of resonance. Mathematically, this means the amplitude of the vibrations becomes unbounded. The prior choice of $X\text{cos}(\omega t)$ for the particular solution fails as it is also a solution for a homogeneous equation. Therefore, a new particular solution is needed for the case where $\omega_\text{n} = \omega$. This particular solution can be written as:
 \begin{equation}
  x_\text{p}(t) = t X\text{sin}(\omega t)
 \end{equation}    
 ...
 \begin{equation}
  x_\text{p}(t) = \frac{f_0}{2 \omega} t \text{sin}(\omega t)
 \end{equation} 
 ...
 \end{equation}   
 Note that $\omega_\text{n}=\omega$; therefore, the frequencies are all in terms of the driving frequency $\omega$. Again, evaluating the solution at $t=0$ for the initial conditions $x_0$ and $v_0$ yields:
 \begin{equation}
...
\begin{vibration_case_study}
\textbf{Resonance and Sound Amplification in Pipe Organs}

\noindent Pipe organs provide a classic and historically important example of resonance being used to amplify vibrations. Long before electronic amplification, organs relied on acoustic resonance to produce sound levels large enough to fill cathedrals and large public spaces. Each organ pipe acts as an acoustic resonator, responding strongly when excited near its natural frequencies. By varying pipe length and geometry, different resonant frequencies are selected, producing the desired musical notes.

\begin{figure}[H]
 \centering
 \includegraphics[]{../figures/pipe_organ}
 \caption{The pipes amplify sound through acoustic resonance of the enclosed air columns\protect\footnotemark[1].}
 \label{fig:pipe_organ}
\end{figure}
\footnotetext[1]{These pipes were formerly installed in the main sanctuary of First Presbyterian Church in downtown Columbia, South Carolina, and were removed and replaced during the 2026 renovation of the sanctuary.}  
...
\pagebreak
 ...
 \noindent First, make sure that the system is not in resonance. Calculating that $\omega_\text{n} = \sqrt{500/10} = 7.07$~rad/s; this shows us that $\omega_\text{n} \neq \omega$. Next, knowing that $f_0 = F_o/m = 10$, we can find the homogeneous and particular solutions as:
 \begin{equation}
  x_\text{h}(t) = A\text{sin}(\omega_\text{n} t + \phi)
 \end{equation}    
 \begin{equation}
  x_\text{p}(t) = X\text{cos}(\omega t) 
 \end{equation} 
 also:   
 \begin{equation}
...
\begin{example}
 ...
 or in standard form:
 \begin{equation}
  \ddot{x}+\omega_\text{n}^2x=f_0\text{sin}(\omega t)
 \end{equation}       
 Note that the forcing function is in terms of sin, not cos as before, so we will have to resolve for the constants $A_1$ and $A_2$. Again, setting the particular solution to $x_\text{p}=X\text{sin}(\omega t)$ and solving for $X$ as before yields:
 \begin{equation}
  x(t) = A_1\text{sin}(\omega_\text{n} t) + A_2\text{cos}(\omega_\text{n} t) + \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{sin}(\omega t)
 \end{equation} 
 ...
 \begin{equation}
  \dot{x}(t) = v_0 = A_1\omega_\text{n}\text{cos}(\omega_\text{n} t) - A_2\omega_\text{n}\text{sin}(\omega_\text{n} t) + \omega\frac{f_0}{\omega_\text{n}^2-\omega^2}\text{cos}(\omega t)
 \end{equation}    
 This allows us to obtain:
 \begin{equation}
  A_1 = \frac{v_0}{\omega_\text{n}}-\frac{\omega}{\omega_\text{n}}\cdot \frac{f_0}{\omega_\text{n}^2-\omega^2}
 \end{equation} 
 at $t=0$. These lead to the full equation for the general solution:
 \begin{equation}
  x(t) = \Big(\frac{v_0}{\omega_\text{n}}-\frac{\omega}{\omega_\text{n}}\cdot \frac{f_0}{\omega_\text{n}^2-\omega^2}\Big)\text{sin}(\omega_\text{n} t) + x_0\text{cos}(\omega_\text{n} t) + \frac{f_0}{\omega_\text{n}^2-\omega^2}\text{sin}(\omega t)
 \end{equation}        
 Also, knowing:
 \begin{equation}
  \omega_\text{n} = \sqrt{\frac{k}{m}} = \sqrt{20} \text{~rad/s} =  4.472 \text{~rad/s}
 \end{equation}    
 and
 \begin{equation}
...
\textbf{Bio-dynamic Induced Loading}
 ...
 When opened on 10 June 2000, 2,000 pedestrians at  1.5 people per square meter used the bridge. The bridge started to rock in the lateral direction at frequencies of between 0.5~Hz and 1.1~Hz with accelerations up to 0.25~g$_\text{n}$. This caused people on the bridge to try to brace themselves by moving their body mass in sync with the bridge's movement. This bio-dynamic coupling created a forced lateral vibration in the bridge that would persist when sufficient people were on the bridge.   
...
\subsection{Harmonic Excitation of Underdamped Systems}

Now that we know a system in resonance without damping will destroy itself, given enough time, its imporatnt to consider the effect of adding damping, as was discussed in Vibration Case Study~\ref{cs:vibration_case_study}. To do this, we will add a damper to the system. 
 \begin{figure}[H]
  \centering
 ...
 \end{figure} 
 \noindent When developing a solution for the EOM that models the system shown in Figure~\ref{fig:1-DOF-spring_dashpot_mass_horizontal_forced_FBD}, let us again (for simplicity) consider a harmonic excitation for $F(t)$ such that:
 \begin{equation}
  F(t) = F_0\text{cos}(\omega t)
 \end{equation}       
 ...
 \begin{equation}
  \ddot{x}(t)+2 \zeta \omega_\text{n} \dot{x}(t) +\omega_\text{n}^2x(t) = f_0\text{cos}(\omega t)
 \end{equation}     
 again, where:
 \begin{equation}
  f_0 = \frac{F_0}{m}
 ...
 \begin{equation}
  \label{eq:x_p(t)}
  x_\text{p}(t) = X \text{cos}(\omega t - \phi_\text{p})
 \end{equation}
 where $\phi_\text{p}$ represents the phase shift. 

 \begin{note}
  $\phi_\text{p}$ is represented in other texts as $\theta$, $\theta_\text{p}$, or even just $\phi$ but we will use $\phi_\text{p}$ throughout the remainder of this text. 
 \end{note}

 Again, the phase shift is expected because of the effect of the damping force. Now, our total equation is:
 \begin{equation}
  x(t) = Ae^{-\zeta \omega_\text{n} t}\text{sin}(\omega_\text{d} t + \phi) +  X \text{cos}(\omega t - \phi_\text{p})
 \end{equation}   
 We can use the method of undetermined coefficients to obtain $X$ and $\phi_\text{p}$ for the particular solution. First, considering that we write the particular solution in the equivalent form:
 \begin{equation}
  x_\text{p}(t) = X \text{cos}(\omega t - \phi_\text{p}) = A_\text{s} \text{cos}(\omega t) + B_\text{s}  \text{sin}(\omega t)
 \end{equation}    
 Taking the derivative of the assumed forms of the particular solution yields:
 \begin{equation}
  x_\text{p}(t) = A_\text{s} \text{cos}(\omega t) + B_\text{s}  \text{sin}(\omega t)
 \end{equation} 
 \begin{equation}
  \dot{x}_\text{p}(t) = -\omega A_\text{s} \text{sin}(\omega t) + \omega B_\text{s}  \text{cos}(\omega t)
 \end{equation}     
 \begin{equation}
  \ddot{x}_\text{p}(t) = -\omega^2 A_\text{s} \text{cos}(\omega t) - \omega^2 B_\text{s}  \text{sin}(\omega t)
 \end{equation}    
 Recall that the homogeneous and particular solutions are each solutions on their own; therefore, the EOM can be used to describe just the particular solution. Substituting $x_\text{p}$. $\dot{x}_\text{p}$, and $\ddot{x}_\text{p}$ for $x$. $\dot{x}$, and $\ddot{x}$ in the EOM in standard form:
 \begin{equation}
   \ddot{x}+2 \zeta \omega_\text{n} \dot{x} +\omega_\text{n}^2x = f_0\text{cos}(\omega t)
 \end{equation}
 yields:
 \begin{equation}
   \big( -\omega^2 A_\text{s} \text{cos}(\omega t) - \omega^2 B_\text{s}  \text{sin}(\omega t) \big)+2 \zeta \omega_\text{n}  \big( -\omega A_\text{s} \text{sin}(\omega t) + \omega B_\text{s}  \text{cos}(\omega t)  \big) +
 \end{equation}
 \begin{equation*}
  \omega_\text{n}^2 \big( A_\text{s} \text{cos}(\omega t) + B_\text{s}  \text{sin}(\omega t) \big) = f_0\text{cos}(\omega t)
 \end{equation*}    
 and rearranging in terms of sin($\omega t$) and cos($\omega t$) yields: 
 \begin{equation}
  (-\omega^2 A_\text{s} + 2 \zeta \omega_\text{n} \omega B_\text{s} + \omega_\text{n}^2 A_\text{s} -f_0) \text{cos}(\omega t) + 
 \end{equation}
 \begin{equation*}
  (-\omega^2 B_\text{s} - 2 \zeta \omega_\text{n} \omega A_\text{s} + \omega_\text{n}^2 B_\text{s})\text{sin}(\omega t) =0
 \end{equation*} 
 ...
 \begin{equation}
  (-2\zeta \omega_\text{n} \omega)A_\text{s} + (\omega_\text{n}^2 - \omega^2)B_\text{s} = 0
 \end{equation} 
 Additionally, at $t=0$, sin($\omega t$)=0 and cos($\omega t$)=1. Therefore, the equation yields  
 \begin{equation}
  (\omega_\text{n}^2 - \omega^2)A_\text{s} + (2\zeta \omega_\text{n} \omega)B_\text{s} = f_0
 \end{equation}    
 We can solve two equations for two unknowns. Writing the two linear equations as the singular matrix equation yields:
 \begin{gather}
\begin{bmatrix}
\omega_\text{n}^2 - \omega^2 & 2\zeta \omega_\text{n} \omega \\
- 2\zeta \omega_\text{n} \omega &  \omega_\text{n}^2 - \omega^2
\end{bmatrix}
\begin{bmatrix}
A_\text{s} \\
B_\text{s}
...
\end{bmatrix}
 ...
 \begin{equation}
  A_\text{s} = \frac{(\omega_\text{n}^2 - \omega^2)f_0}{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}
 \end{equation} 
 \begin{equation}
  B_\text{s} = \frac{2\zeta \omega_\text{n} \omega f_0}{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}
 \end{equation} 
 From trigonometric relationships, we can see that, 
 \begin{equation}
  X = \sqrt{A_\text{s}^2 + B_\text{s}^2}
 \end{equation} 
 \begin{equation}
  \phi_\text{p} = \tan^{-1}\bigg(\frac{B_\text{s}}{A_\text{s}}\bigg)
 \end{equation} 
 We can now derive values for our particular solution $x_\text{p}$:
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} 
  \label{eq:X_damped}
 \end{equation} 
 \begin{equation}
  \phi_\text{p} = \tan^{-1} \bigg(\frac{2\zeta \omega_\text{n} \omega}{\omega_\text{n}^2 - \omega^2}\bigg)
 \end{equation}    
 Now we can build a solution for the particular equation ($x_\text{p}$), therefore, the total solution becomes:
 \begin{equation}
  x(t) = x_\text{h}(t) + x_\text{p}(t)
 \end{equation}
 \begin{equation}
 ...
 \begin{equation}
  A = \frac{x_0 -X \text{cos}(\phi_\text{p})}{\text{sin}(\phi)}
 \end{equation}    
 \begin{equation}
  \phi =  \tan^{-1} \bigg(\frac{\omega_\text{d} \big( x_0 -X \text{cos}(\phi_\text{p})\big)}{v_0 + \big(x_0 - X \text{cos}(\phi_\text{p})\big) \zeta \omega_\text{n} - \omega X \text{sin}(\phi_\text{p}) }\bigg)
 \end{equation}   
 ...
 \end{equation}    
 \begin{equation}
  \phi =  \text{tan}^{-1}\bigg(\frac{\omega_\text{d} ( x_0 -X \text{cos}(\phi_\text{p}))}{v_0 + (x_0 - X \text{cos}(\phi_\text{p})) \zeta \omega_\text{n} - \omega X \text{sin}(\phi_\text{p}) }\bigg)
 \end{equation} 
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} 
 \end{equation} 
 ...
 \begin{example}
  ...
  \label{ex:Plotting_Steady_State} 
  ...
  \noindent Configuration b increases the forcing function $F_0$ to 3 N. This results in a similar response to configuration a, but with a linearly scaled amplitude:
  \vspace{-3ex}
  \begin{figure}[H]
   \centering
...
\subsection{Frequency Response of Underdamped Systems}      
 From equations \ref{eq:damped_forced_x} through \ref{eq:damped_forced_theta_p} and the figures in example \ref{ex:Plotting_Steady_State}, we can see that for larger values of $t$, the transient response dies out while only the steady-state response controls the displacement of the total response. This is always true if the system has any significant damping. Therefore, it is often prudent to ignore the transient part and focus only on the steady-state response. Considering the equation for the particular solution: 
 \begin{equation}
  x_\text{p}(t) = X \text{cos}(\omega t - \phi_\text{p})
 \end{equation}    
 and knowing the values for $X$ and $\phi_\text{p}$: 
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} 
 \end{equation} 
 \begin{equation}
 ...
 \begin{equation}
  r = \frac{\omega}{\omega_\text{n}}
 \end{equation}
 Another common way to express $r$ is $\beta$. Next, Recall that:
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}}  = \frac{\frac{F_0}{m}}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} 
 \end{equation}    
 If we factor out $\omega_\text{n}^2$ from the denominator and substitute in $\omega_\text{n}^2 = k/m$ and $r = \omega/\omega_\text{n}$, we get:
 \begin{equation}
  X = \frac{\frac{F_0}{m}}{\omega_\text{n}^2 \sqrt{\big(1 - (\frac{\omega}{\omega_\text{n}})^2\big)^2 +  (2\zeta \frac{\omega}{\omega_\text{n}})^2}} =  \frac{\frac{F_0}{k}}{\sqrt{(1-r^2)^2+(2\zeta r)^2}}
 \end{equation}    
 this becomes:
 \begin{equation}
  \frac{Xk}{F_0} = \frac{X \omega_\text{n}^2}{f_0} = \frac{1}{\sqrt{(1-r^2)^2+(2\zeta r)^2}}
 \end{equation}    
 In a similar fashion, if we manipulate the equation for $\phi_\text{p}$, we can get $\phi_\text{p}$ in terms of $r$:
 \begin{equation}
  \phi_\text{p} = \tan^{-1} \bigg(\frac{2 \zeta r}{1-r^2}\bigg)
 \end{equation} 
 ...
 \begin{figure}[H]
  ...
  \caption{Normalized amplitude response for frequency ratio ($r$= ($\omega/\omega_\text{n}$)) from 0 to 2 for a variety of critical damping ratios.}
  \label{fig:underdamped_frequency_response_amplitude}
 \end{figure}
 % I have seen expressions that you take k out of the units on the y-axis, the base is 1/k. However, I don't see the value of it.   
...
\begin{example}
 ...
 \end{figure}    
 \noindent where $\omega_\text{n} = 132$~rad/s and $\zeta$ = 0.0085. Calculate the displacements of the steady-state response for $\omega$=132 and 125~rad/s. In both cases, use $f_0$ = 10 N/kg. 
 ...
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} = \frac{10}{2(0.0085)(132)^2} = 0.034 \text{ m}
 \end{equation}       
 while for $\omega$=125~rad/s X is:
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} = \frac{10}{\sqrt{(1799)^2 +  (280.5)^2}}  = 0.005 \text{ m}
 \end{equation}    
...
\begin{example}
 ...
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (2\zeta \omega_\text{n} \omega)^2}} 
 \end{equation} 
 and recalling from the EOM in standard form that $2\zeta \omega_\text{n} = c/m$ we can obtain:
 \begin{equation}
  X = \frac{f_0}{\sqrt{(\omega_\text{n}^2 - \omega^2)^2 +  (\frac{c}{m} \omega)^2}} 
 \end{equation}  
 rearranging for $c$ gives:  
 \begin{equation}
  c = m\sqrt{\frac{f_0^2}{\omega^2 X^2}-\frac{\big(\omega_\text{n}^2-\omega^2\big)^2}{\omega^2}} = \sqrt{\frac{F_0^2}{\omega^2 X^2}-m^2\frac{\big(\omega_\text{n}^2-\omega^2\big)^2}{\omega^2}} 
 \end{equation}
...
\subsection{Base Excitation}
 ...
 \begin{equation}
 y(t) = Y\text{sin}(\omega_\text{b} t)
 \label{eq:Ysinomegat}
 \end{equation}
 Taking the derivative of the assumed input yields:
 \begin{equation}
 \dot{y}(t) = Y \omega_\text{b} \text{cos}(\omega_\text{b} t)
 \end{equation}
 where $Y$ is the amplitude and $\omega_\text{b}$ is the frequency of the base excitation. Adding these terms to our EOM yields:
 \begin{equation}
 m\ddot{x} + c\dot{x} + kx = c Y \omega_\text{b} \text{cos}(\omega_\text{b} t)  + k Y\text{sin}(\omega_\text{b} t)  
 \end{equation}
 ...
 \begin{equation}
 \ddot{x} + 2 \zeta \omega_\text{n} \dot{x} + \omega_\text{n}^2x = 2 \zeta \omega_\text{n} \omega_\text{b} Y \text{cos}(\omega_\text{b} t)  + \omega_\text{n}^2 Y\text{sin}(\omega_\text{b} t)
 ...
 \begin{equation}
 \ddot{x} + 2 \zeta \omega_\text{n} \dot{x} + \omega_\text{n}^2x = C \text{cos}(\omega_\text{b} t)  + D \text{sin}(\omega_\text{b} t)  
...
\begin{vibration_case_study}
 ...
 \footnotetext[1]{Image cropped from original photo by Leggi il Firenze, CC BY 3.0 <https://creativecommons.org/licenses/by/3.0>, via Wikimedia Commons}       
...
\pagebreak
 ...
 \subsubsection{Displacement Transmissibility Solution for Base Excitation}
  The steady-state solution is often more important than the transient solution when designing systems for continuous use. The particular solution for the base excited system annotated in figure \ref{fig:1_DOF_spring_dashpot_mass_vertical_base_excited_FBD} with the EOM presented in equation \ref{eq:standard_form_base_excitation_CD} can be expressed as $ x_\text{p}(t)$. To solve for this expression, we will use the linearity of the system and solve for a solution that is the sum of two particular solutions. Resulting in:
  \begin{equation}
 x_\text{p}(t) =  x_\text{p}^{(1)}(t) +  x_\text{p}^{(2)}(t)  
  \end{equation}
 Recall that the steady state solution for a harmonically excited spring-mass-damper can be expressed as $x_\text{p}(t) = X\text{cos}(\omega t - \phi_\text{p})$, as denoted in equation~\ref{eq:x_p(t)}. For the base excitation problem, we will convert this expression to $x_\text{p}(t) = X\text{cos}(\omega_\text{b} t - \phi_1)$. Therefore, for a base-excited problem, the forcing function can be expressed as the sum of particular solutions:
  \begin{equation}
   C \text{cos}(\omega_\text{b} t)  + D \text{sin}(\omega_\text{b} t)   = x_\text{p} =  x_\text{p}^{(1)} +  x_\text{p}^{(2)} 
  \end{equation}
  where we dropped the $(t)$ term from the expression for simplicity. We can then write:
  \begin{equation}
   x_\text{p}^{(1)} = X^{(1)}\text{cos}(\omega_\text{b} t - \phi_1)
  \end{equation}
  \begin{equation}
   x_\text{p}^{(2)} = X^{(2)} \text{sin}(\omega_\text{b} t - \phi_1)
  \end{equation}
 
  \begin{note}
  $x_\text{p}^{(1)}$ uses a cos term while $x_\text{p}^{(2)}$ uses a sin term. Both solutions use $\phi_1$ as the damping term, as the phase angle is independent of the excitation amplitude, and the sin and cos terms account for the difference in phase. 
  \end{note}

  For $x_\text{p}^{(1)}$, we again use the method of undetermined coefficients to obtain a solution for $x_\text{p}^{(1)} = X^{(1)}\text{cos}(\omega_\text{b} t - \phi_1)$. This can be as simple as setting $2 \zeta \omega_\text{n} \omega_\text{b} Y$ equal to $f_0$ from equation \ref{eq:X_damped} that defines $X$ for underdamped systems. Again, $2 \zeta \omega_\text{n} \omega_\text{b} Y$  comes from the EOM in standard form as presented in the equation  
  ...
  \begin{equation}
   x_\text{p}^{(1)} = \frac{2 \zeta \omega_\text{n} \omega_\text{b} Y}{\sqrt{(\omega_\text{n}^2 - \omega_\text{b}^2)^2 +  (2\zeta \omega_\text{n} \omega_\text{b})^2}}  \text{cos}(\omega_\text{b} t - \phi_1)
   \label{eq:xp_1}
  \end{equation}
  where:
  \begin{equation}
   \phi_1 = \tan^{-1} \bigg(\frac{2\zeta \omega_\text{n} \omega_\text{b}}{\omega_\text{n}^2 - \omega_\text{b}^2}\bigg)
  \end{equation}
  
  Next, the particular solution associated with $x_\text{p}^{(2)} = X^{(2)} \text{sin}(\omega_\text{b} t - \phi_1)$ can be obtained using the same method of undetermined coefficients and setting $f_0$ from equation \ref{eq:X_damped} to the driving force for $x_\text{p}^{(2)}$ in equation  \ref{eq:standard_form_base_excitation}, $\omega_\text{n}^2$. This results in:
  \begin{equation}
   x_\text{p}^{(2)} = \frac{\omega_\text{n}^2 Y}{\sqrt{(\omega_\text{n}^2 - \omega_\text{b}^2)^2 +  (2\zeta \omega_\text{n} \omega_\text{b})^2}}  \text{sin}(\omega_\text{b} t - \phi_1)
   \label{eq:xp_2}
  \end{equation}
  As both equation \ref{eq:xp_1} and \ref{eq:xp_2}  have the same argument $(\omega_\text{b} t - \phi_1)$, these can be added as:
  \begin{equation}
   x_\text{p} =  x_\text{p}^{(1)} + x_\text{p}^{(2)}
  \end{equation}
  to obtain:
  \begin{equation}
   x_\text{p} =  \omega_\text{n} Y   \sqrt{\frac{\omega_\text{n}^2 + (2 \zeta \omega_\text{b})^2 }{(\omega_\text{n}^2 - \omega_\text{b}^2)^2 +  (2\zeta \omega_\text{n} \omega_\text{b})^2} }  \text{cos}(\omega_\text{b}t - \phi_1 - \phi_2)
  \end{equation}
  and:
  \begin{equation}
   \phi_2 = \tan^{-1} \bigg(\frac{\omega_\text{n}}{2\zeta \omega_\text{b}}\bigg)
  \end{equation}
  ...
  \begin{equation}
  r=\frac{\omega_\text{b}}{\omega_\text{n}}
  \end{equation} 
  ...
  \begin{example}
   ...
   \end{figure}    
   \noindent where $k$ = 300,000~N/m, $m$ = 1600~kg, $c$ = 15,000~kg/s, the period of road roughness = 3 m, and the height of road roughness = 0.01 m. What is the deflection experienced by the car at $v$ = 50 km/h?
   ...
   \end{equation}     
   \begin{equation}
    v \text{ m/s} = 50 \text{ km/hr}\Bigg(\frac{1000 \text{ m}}{1 \text {km}}\Bigg) \Bigg(\frac{1 \text{ hours}}{3600 \text { s}}\Bigg) = 13.88 \text{ m/s}
   \end{equation}  
   \begin{equation}
    \omega_\text{b} = \Bigg(\frac{ 13.88 \text{ m}}{s}\Bigg) \Bigg(\frac{ 1 \text{ cycle}}{3 \text{ m}}\Bigg) \Bigg(\frac{ 2 \pi \text{ rad}}{\text {cycle}}\Bigg) = \text{~rad/s} = 29.08 \text{~rad/s} 
   \end{equation}  
   ...
   \begin{equation}
    \omega_\text{n} = \sqrt{\frac{k}{m}} = \sqrt{\frac{300,000}{1600}} = 13.69 \text{~rad/s}
   \end{equation}    
   Therefore:
   \begin{equation}
   r=\frac{\omega_\text{b}}{\omega_\text{n}}  = 2.124
   \end{equation}   
   and:
   \begin{equation}
...
\begin{example}
\label{ex:Acceleration_to_Displacement}
\textbf{Converting Harmonic Base Acceleration to Displacement}

\noindent In many engineering applications, base motion is specified in terms of acceleration rather than displacement. Examples include shaker-table testing of aerospace components, aircraft structures subjected to engine-induced vibration, rotating machinery mounted on flexible supports, and vehicles experiencing base motion from uneven terrain.
Suppose the base acceleration is given as
\begin{equation}
\ddot{y}(t) = A \cos(\omega_\text{b} t)
\end{equation}
where $A$ is the acceleration amplitude and $\omega_\text{b}$ is the excitation frequency. Determine the displacement amplitude $Y$ in terms of the acceleration amplitude $A$.

\noindent\textbf{Solution:} \\
 The displacement transmissibility solution derived previously requires the base motion in the form
\begin{equation}
y(t) = Y \sin(\omega_\text{b} t),
\end{equation}
as originally presented in Equation~\ref{eq:Ysinomegat}. To obtain the displacement, integrate the acceleration term 
\begin{equation}
\ddot{y}(t) = A \cos(\omega_\text{b} t)
\end{equation}
twice. Integrating once gives the velocity:
\begin{equation}
\dot{y}(t) = \frac{A}{\omega_\text{b}} \sin(\omega_\text{b} t) + C_1.
\end{equation}
Integrating a second time gives the displacement:
\begin{equation}
y(t) = -\frac{A}{\omega_\text{b}^2} \cos(\omega_\text{b} t) + C_1 t + C_2.
\end{equation}
...
\begin{equation}
y(t) = -\frac{A}{\omega_\text{b}^2} \cos(\omega_\text{b} t).
\end{equation}
Thus, the displacement amplitude is
\begin{equation}
Y = \frac{A}{\omega_\text{b}^2}.
\end{equation}
This demonstrates an important harmonic relationship: for sinusoidal motion, the acceleration amplitude equals $\omega_\text{b}^2$ times the displacement amplitude. Consequently, higher-frequency ground motion produces larger accelerations but smaller displacements for the same amplitude $A$.
\end{example}

\begin{note}
In practice, recovering displacement from measured acceleration data by double integration is difficult, as sensor noise and bias are amplified and can produce significant drift. When the motion can be reasonably approximated as harmonic, using $Y = A/\omega_\text{b}^2$ as shown in Example~\ref{ex:Acceleration_to_Displacement} provides a simple way to estimate displacement amplitude without performing numerical integration.
\end{note}
 ...
 \subsubsection{Force Transmissibility Solution for Base Excitation}
  ...
  \begin{equation}
   x_\text{p}(t) =  \omega_\text{n} Y   \sqrt{\frac{\omega_\text{n}^2 + (2 \zeta \omega_\text{b})^2 }{(\omega_\text{n}^2 - \omega_\text{b}^2)^2 +  (2\zeta \omega_\text{n} \omega_\text{b})^2} }  \text{cos}(\omega_\text{b}t - \phi_1 - \phi_2)
  \end{equation} 
  if we differentiate this twice, to obtain $\ddot{x}(t)$ and combine this with $F(t) = -m\ddot{x}(t)$ we get:
  \begin{equation}
   F(t) =  m \omega_\text{b}^2 \omega_\text{n} Y   \sqrt{\frac{\omega_\text{n}^2 + (2 \zeta \omega_\text{b})^2 }{(\omega_\text{n}^2 - \omega_\text{b}^2)^2 +  (2\zeta \omega_\text{n} \omega_\text{b})^2} }  \text{cos}(\omega_\text{b}t - \phi_1 - \phi_2)
  \end{equation} 
  ...
  this becomes:
  \begin{equation}
   F(t) =  F_\text{T} \text{cos}(\omega_\text{b}t - \phi_1 - \phi_2)
  \end{equation} 
  where $F_\text{T}$ is the magnitude of the transmitted force and is 
  \begin{equation}
  ...
  \begin{figure}[H]
   ...
   \caption{Force transmissibility for an underdamped 1-DOF system.}
	\label{fig:force_transmissibility}
  \end{figure}
...
\pagebreak
 ...
 \pagebreak
  ...
  \begin{example}
   \textbf{Force-Based Design of a Steel Platform}

\noindent A steel platform in an industrial facility is modeled as a rigid deck of mass $m$ supported by steel columns with stiffness $k$ and damping $c$ (5\% damping provided through the columns). The platform is subjected to harmonic excitation from nearby rotating machinery such that $r=2$. During a design review, it was found that the force in a critical bolted connection slightly exceeds its allowable limit. A team member proposes increasing the damping in the columns to reduce the force.
Based on force considerations alone, do you agree?


\begin{figure}[H]
\centering
\includegraphics[]{../figures/base_excited_lateral_structure.png}
\caption{Idealized 1-DOF model of the steel platform and supports.}
\end{figure}

\noindent\textbf{Solution:} \\
\noindent First, let us put the steel platform into a form we recognize. The idealized model is shown in Figure~\ref{key:wall_excited_undamped-system}, where the deck displacement $x$ and the ground displacement $y$ act in the same direction. 
From our work in Chapter 1, we know that this configuration is equivalent to the vertical spring-mass system, except that the support is moving. Therefore, the equation of motion takes the familiar base-excitation form
\begin{equation}
m\ddot{x} + c\dot{x} + kx = c\dot{y} + ky,
\end{equation}
...
\begin{figure}[H]
\centering
\includegraphics[]{../figures/wall_excited_damped-system.png}
\caption{A base excited 1-DOF spring-mass system.}
\label{key:wall_excited_undamped-system}
\end{figure}


From the force-transmissibility plot (Fig.~\ref{fig:force_transmissibility}), at $r=2$ increasing damping increases the transmitted force. Thus, adding damping to reduce connection force would be counterproductive in this case, and you should politely disagree with your colleague's proposal.

A more effective approach is to reduce the frequency ratio $r$ by increasing the natural frequency, thereby shifting the system to the left in Fig.~\ref{fig:force_transmissibility}.
\begin{equation}
\omega_\text{n}=\sqrt{\frac{k}{m}},
\end{equation}
since $r=\omega_\text{b}/\omega_\text{n}$. This can be achieved by increasing the stiffness $k$ or decreasing the mass $m$.
...
%
%
% %\lstinputlisting[caption = {Sample code from Matlab}]{code/matlab_eigenvalue_\text{p}roblem.m}
...
\end{lstlisting}
   ...
   \begin{figure}[H]
    ...
    \caption{Displacement response of the 1-DOF system in figure~\ref{fig:2-DOF-spring_mass_horizontal_3}.}
