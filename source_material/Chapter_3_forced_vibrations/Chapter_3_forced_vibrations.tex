...
\begin{example}
 ...
 \noindent First, make sure that the system is not in resonance. Calculating that $\omega_n = \sqrt{500/10} = 7.07$~rad/s; this shows us that $\omega_n \neq \omega$. Next, knowing that $f_0 = F_o/m = 10$, we can find the homogeneous and particular solutions as:
 \begin{equation}
...
\begin{example}
 ...
 Also, knowing:
 \begin{equation}
  \omega_n = \sqrt{\frac{k}{m}} = \sqrt{20} \text{~rad/s} =  4.472 \text{~rad/s}
 \end{equation}    
 and
 \begin{equation}
...
\begin{example}
 ...
 \end{figure}    
 \noindent where $\omega_n = 132$~rad/s and $\zeta$ = 0.0085. Calculate the displacements of the steady-state response for $\omega$=132 and 125~rad/s. In both cases, use $f_0$ = 10 N/kg. 
...
\subsection{Base Excitation}
 ...
 \end{equation}
 where $Y$ is the amplitude and $\omega_b$ is the frequency of the base excitation. Adding these terms to our EOM yields:
 \begin{equation}
...
\begin{equation}
\ddot{y}(t) = A \cos(\omega_b t)
\end{equation}
where $A$ is the acceleration amplitude and $\omega_b$ is the excitation frequency. Determine the displacement amplitude $Y$ in terms of the acceleration amplitude $A$.
...
\begin{equation}
\dot{y}(t) = \frac{A}{\omega_b} \sin(\omega_b t) + C_1.
\end{equation}
Integrating a second time gives the displacement:
\begin{equation}
y(t) = -\frac{A}{\omega_b^2} \cos(\omega_b t) + C_1 t + C_2.
\end{equation}
...
\begin{equation}
y(t) = -\frac{A}{\omega_b^2} \cos(\omega_b t).
\end{equation}
Thus, the displacement amplitude is
\begin{equation}
Y = \frac{A}{\omega_b^2}.
\end{equation}
This demonstrates an important harmonic relationship: for sinusoidal motion, the acceleration amplitude equals $\omega_b^2$ times the displacement amplitude. Consequently, higher-frequency ground motion produces larger accelerations but smaller displacements for the same amplitude $A$.
\end{example}

\begin{note}
In practice, recovering displacement from measured acceleration data by double integration is difficult, as sensor noise and bias are amplified and can produce significant drift. When the motion can be reasonably approximated as harmonic, using $Y = A/\omega_b^2$ as shown in Example~\ref{ex:Acceleration_to_Displacement} provides a simple way to estimate displacement amplitude without performing numerical integration.
\end{note}
