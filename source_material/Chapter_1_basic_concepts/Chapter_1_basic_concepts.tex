...
\begin{document}
 ...
 \section{Basic Concepts in Vibrations}
    ...
    The consideration of vibrations and their associated mathematical modeling are important factors in the design of mechanical systems. In this text, the fundamental theories of vibration are presented and modeled using basic physical principles such as Newton's three laws of motion. These models are analyzed using the mathematical tools of calculus and differential equations. 

 \begin{vibration_case_study}
  
   \textbf{TSR-2 and the Resonance of the Human Eye}

  \noindent Why study vibrations? One day, it could save your life! The British Aircraft Corporation (BAC) TSR-2 (figure~\ref{fig:TSR_2}) was a strike and reconnaissance aircraft developed during the Cold War by BAC, for the Royal Air Force (RAF).  During the second flight test of airframe XR219, vibration from one of the plane's fuel pumps caused vibration at the resonant frequency of the human eyeball. As you may expect, a human eye experiencing high levels of vibrations will distort, causing blindness. Test pilot Roland Beamont was blinded by the vibrations that originated in the fuel pump and transmitted to his head. Roland just happened to be an expert in vibrations and had the knowledge to throttle back one engine. This led to a reduction in the vibrations and a restoration of his full vision. 
 
  \begin{figure}[H]
   \vspace{-2.5ex}
   \centering
   \includegraphics[width=4.7in]{../figures/TSR_2.jpg}
   \vspace{-1ex}
  ...
  Roland gained his expertise in vibrations during World War II. During this time, he led the vibration program of the Hawker Typhoon. He fit vibrographs to airplanes to determine the effectiveness of propeller balancing. He also led the testing of seats with vibration isolators to limit vibrations transmitted from the airframe to the pilot. 
 ...
 \subsection{Single Degree-of-Freedom Systems}
  ...
  \noindent The assumption of small displacements states that any displacement in a system is considerably smaller than the initial geometry of the system. This means that any 2\textsuperscript{nd}-order effects caused by displacements within the system are ignored. These 2\textsuperscript{nd}-order effects could be loads or angles at the point of linkage/spring connections. 
  \end{review}
...
\pagebreak
  ...
  \begin{vibration_case_study}
   ...
   \noindent Why study vibrations? Because vibrations form an integral part of how we interact with our world, and as such, are an important consideration in a product. For example, vibrations in the automotive industry fall within a field of expertise termed Noise Vibration and Harshness (NVH). NVH is important because, within a single company, different levels of NVH will be desired for different market segments and products. 
 ...
 \subsection{Equivalent Stiffness}
  ...
  \noindent Hooke's Law states that the force ($F$) needed to extend or compress a spring by some distance $x$ scales linearly with respect to that distance. This law can be extended to the tensional stress of a uniform and elastic bar where the length, area, and Young's modulus of the bar are represented by $l$, $A$, and $E$, respectively. Knowing the tensile stress in the bar
   ...
   F = -kx
   \end{equation}    
   since the direction of the restoring force is opposite to the spring displacement.
  \end{review}
...
\pagebreak
  \subsubsection{Equivalent Stiffness of Structural Systems} 
   ...
   \pagebreak 
   In a similar fashion, we can also solve the equivalent system for a mass at the end of a cantilever beam (figure~\ref{fig:spring_and_bar_mass_cantilever_beam}).
   \begin{figure}[H]
    \centering
  ...
  \subsubsection{Springs in Series and Parallel}
   ...
   In many cases, it becomes necessary to model a mechanical system as a set of springs (e.g., a composite material, a table with multiple legs).  For these systems, or for systems with more than one spring acting on a body, equivalent stiffness can be calculated as shown in figure~\ref{fig:equivalent_stiffness}.
   \begin{figure}[H]
    \centering
    ...
    \label{fig:equivalent_stiffness}
   \end{figure} 
   ...
   \noindent Calculate the equivalent stiffness of the following system:
    ...
    \end{figure} 
    The springs are combined as shown, using the equations defined before.  Now, considering that the displacement ($\delta$) of the top spring and the bottom springs are the same, we can state the total stiffness $k$, which is the summation of the two. Therefore,    
    \begin{figure}[H]
     \centering
     \includegraphics[]{../figures/equivalent_mass_and_spring_system_2}
     \caption{A spring-mass system simplified down from springs in series and parallel.}
    \end{figure} 
...
\pagebreak
  ...
  \begin{example}   
   ...
   m\ddot{x} = -kx.
   \end{equation}    
   \textbf{Step-3} Rearrange the Equation to construct an EOM 
   \begin{equation}
   m\ddot{x} + kx = 0.
   \end{equation}  
  \end{example}   
  ...
  \begin{review}
   ...
   \noindent Isaac Newton and Robert Hooke were contemporaries, working in England in the late 1600s, though at different institutions and often in competition rather than collaboration. Hooke made early and important contributions to elasticity and motion, while Newton later developed the laws of motion that formalized much of classical mechanics. Newton's famous remark about ``If I have seen further, it is by standing on the shoulders of giants'' is often read as acknowledging prior work, though it may also have been a subtle jab at Hooke and his academic circle\footnotemark[1].

   \begin{figure}[H]
     \centering
     \includegraphics[width=6in]{../figures/Newton_Hooke}\\
     \caption{portraits of: (a) Isaac Newton~[2] and (b) Robert Hooke~[3].}
    \end{figure}
    \footnotetext[1]{Reiss, H. E. ``On the shoulders of giants.'' Journal of Obstetrics and Gynaecology 20.2 (2000): 185-187.} 
    \footnotetext[2]{Godfrey Kneller, Public domain, via Wikimedia Commons}
    \footnotetext[3]{Science History Institute, Public domain, via Wikimedia Commons}
     \end{review}
...
\pagebreak
  ...
  \begin{vibration_case_study}
   ...
   \begin{figure}[H]
    \centering
    \includegraphics[width=4.5in]{../figures/Ling_Temco_Vought_XC_142A.jpg}
...
\pagebreak
   ...
   \end{equation}
   We can then use the information from the equilibrium state to cancel out some terms, which becomes
   \begin{equation}
    m\ddot{x} = -kx.
   \end{equation}    
   \textbf{Step-3} Rearrange the Equation to construct an EOM
  ...
  \begin{example} 
   ...
   \noindent \textbf{Step-2} Apply Newton's 2nd law, given the fact that we were given the moment of inertia of a disk as $J$, therefore
   \begin{equation}
    \curveplusclock \sum M_\text{O} = J \ddot{\theta} =  -k \theta.
   \end{equation}
   
   \noindent \textbf{Step-3} Derive EOM to get
   \begin{equation}
