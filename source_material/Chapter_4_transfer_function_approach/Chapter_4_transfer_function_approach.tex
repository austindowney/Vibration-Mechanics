...
\begin{document}
	...
	\section{Transfer Function Approach}
		...
		\subsection{Transfer Function Method for Generic Systems}
...
\end{review}
		...
		\subsection{Transfer Function Method for Solving Vibrating Systems}
			...
			\subsubsection{Free Response of an Undamped System}
			...
			\begin{equation}
				\big[s^2X(s) -sx_0 -v_0 \big] + \big[ \omega_\text{n}^2X(s) \big] =0
			\end{equation}
			...
			\begin{equation}
			X(s) = \frac{sx_0 + v_0}{s^2 + \omega_\text{n}^2}
			\end{equation}
			We can expand this form of $X(s)$ to obtain equations listed in our Laplace Transform table:
			\begin{equation}
			X(s) = \frac{sx_0}{s^2 + \omega_\text{n}^2} + \frac{v_0}{s^2 + \omega_\text{n}^2}\cdot \frac{\omega_\text{n}}{\omega_\text{n}}
			\end{equation}
			This becomes:
			\begin{equation}
			X(s) = x_0\frac{s}{s^2 + \omega_\text{n}^2} + \bigg(\frac{v_0}{\omega_\text{n}}\bigg) \cdot \frac{\omega_\text{n}}{s^2 + \omega_\text{n}^2}
			\end{equation}
			...
			The same procedure can be used to calculate the underdamped and forced responses. However, when calculating these responses the algebraic solution for $X(s)$, $s$ often contains quotients of polynomials. These Polynomial ratios may not be found in simple Laplace tables and must be solved using the method of partial fractions. An example of this procedure can be found in Appendix B of Inman\protect\footnotemark[1]. 
...
\subsubsection{Impulse Response of a Spring-Mass System}
...
The impulse response function can be solved for analytically, however, we will solve it using the transfer function approach. Here we will consider the underdamped spring-mass system. First, assume that the system is at rest (no initial conditions). Next, we write the EOM as:
\begin{equation}
...
\begin{equation}
X(s) = \frac{1}{m} \cdot \frac{1}{s^2 + 2 \zeta \omega_\text{n} s + \omega_\text{n}^2}
\end{equation}
...
\begin{equation}
x(t) = \frac{1}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} t} \text{sin}(\omega_\text{d}t)
...
\begin{equation}
x(t) = \frac{1}{m \omega_\text{n}}\text{sin}(\omega_\text{n} t)
\end{equation}
...
\subsubsection{Step Response of a Spring-Mass System}
...
For an undamped spring-mass system subjected to a unit step, assuming both initial conditions are zero, the solution can be obtained using the transform method. First, the EOM is 
\begin{equation}
...
\begin{equation}
	X(s) = \frac{1}{m} \cdot \frac{1}{s(s^2+\omega_\text{n}^2)}
\end{equation}
...
\begin{equation}
	x(t) = \frac{1}{m} \cdot \frac{1}{\omega_\text{n}^2}\big(1-\text{cos}(\omega_\text{n} t)\big) = \frac{1}{k}\big(1-\text{cos}(\omega_\text{n} t)\big)
	\label{eq:step_undamped}
\end{equation}
 
For a underdamped spring-mass-damper system subjected to a unit step, assuming both initial conditions are zero, the solution can be obtained using the transform method. First, the EOM is:
\begin{equation}
m\ddot{x}(t) + c\dot{x}(t) + kx(t) = \Phi(t)
\end{equation}
Converting to the standard form results in:
\begin{equation}
\ddot{x}(t) + 2  \zeta \omega_\text{n}\dot{x}(t) + \omega_\text{n}^2x(t) = \frac{1}{m} \cdot \Phi(t)
\end{equation}
taking the Laplace transform of both sides and assuming zero initial conditions yields:
\begin{equation}
	s^2X(s) + 2  \zeta \omega_\text{n} s X(s) + \omega_\text{n}^2 X(s)  =\frac{1}{m} \cdot \frac{1}{s}
\end{equation}
Next, this equation is solved for $X(s)$ as:
\begin{equation}
	X(s) = \frac{1}{s^2 + 2  \zeta \omega_\text{n} s + \omega_\text{n}^2} \cdot \frac{1}{m} \cdot \frac{1}{s}
\end{equation}
multiplying the right-hand-side of this equation by  $\frac{\omega_\text{n}^2}{\omega_\text{n}^2}$ results in:
\begin{equation}
	X(s) = \frac{1}{m \omega_\text{n}^2} \cdot \frac{\omega_\text{n}^2}{s(s^2+2\zeta\omega_\text{n} s+\omega_\text{n}^2)}
\end{equation}
Again, the $\frac{1}{m\omega_\text{n}^2}$ will pass through the Laplace function. Therefore, taking the inverse Laplace transform using \#11 on the Laplace transform sheet yields:
\begin{equation}
	x(t) = \frac{1}{m \omega_\text{n}^2} \cdot \Big(1 - \frac{\omega_\text{n}}{\omega_\text{d}}e^{-\zeta \omega_\text{n} t}\text{sin}(\omega_\text{d}t + \phi)\Big)\text{, where } \phi = \text{cos}^{-1}(\zeta)\text{, where } \zeta<1
\end{equation}
After obtaining equations for the undamped and underdamped cases, the responses for the unit step, solved with the transform method, can be plotted as:
\begin{figure}[H]
	\centering
...
\begin{example}
\label{ex:Displacement_under_Dynamic_Loading}
...
\noindent Setting the load applied to the truck as 1 unit, it can be seen that this is a unit step loading condition with an undamped system. 
From Equation~\ref{eq:step_undamped} we know the expression for the transient and steady-state displacement of the truck bed. Therefore,
\begin{equation}
	x(t) = \frac{1}{k}\big(1-\text{cos}(\omega_\text{n} t)\big) = \frac{m g}{k}\big(1-\text{cos}(\omega_\text{n} t)\big).
\end{equation}
This equation has a maximum amplitude when the cos$(\omega_\text{n} t)=-1$, resulting in
\begin{equation}
...
%\textbf{Example 1 - underdamped}
%For the case where the truck has a damper with a value of $\zeta=0.5$, the displacement $(x)$ can be modeled as:
%\begin{equation}
%	x(t) = \frac{m g}{m \omega_\text{n}^2} \cdot \Big(1 - \frac{\omega_\text{n}}{\omega_\text{d}}e^{-\zeta \omega_\text{n} t}\text{sin}(\omega_\text{d}t + \phi)\Big)\text{, where } \phi = \text{cos}^{-1}(\zeta)
%\end{equation}

\end{example}

\begin{note}
	A useful rule of thumb for engineers and designers is that dynamic loading can produce forces up to twice the static load for an undamped step input (and less if damping is applied). As shown in Example~\ref{ex:Displacement_under_Dynamic_Loading}, this follows from linear behavior and Hooke's law ($F = kx$), where a peak displacement twice the static value implies a corresponding doubling of force.
\end{note}
...
\subsection{Response to Arbitrary Inputs}
...
\begin{equation}
g(t) = x(t) =\frac{1}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} t} \text{sin}(\omega_\text{d}t)
...
\begin{equation}
g(t-\tau) = \frac{1}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} (t-\tau)} \text{sin}\big(\omega_\text{d} (t-\tau)\big), \;  t \ge \tau
...
\begin{equation}
	x(t) = \frac{1}{m \omega_\text{d}} \int_{0}^{t} F(\tau) e^{-\zeta \omega_\text{n} (t-\tau)} \text{sin}\big(\omega_\text{d}(t-\tau)\big) d \tau
	\label{eq:Duhamel_2}	
\end{equation}
...
\label{example:Double_Hammer_Impact}
	...
	Solving this equation for $X(s)$:
	\begin{equation}
		X(s) = \frac{0.2}{m} \cdot \frac{1}{s^2 + 2 \zeta \omega_\text{n} s + \omega_\text{n}^2}
	\end{equation}
	Again, consulting \#10 in the table for Laplace transforms results in:
	\begin{equation}
		x_1(t) = \frac{0.2}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} t} \text{sin}(\omega_\text{d}t)
	\end{equation}
	...
	\end{equation}
	\begin{equation}
		x_2(t) = \frac{0.1}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} (t-\tau)} \text{sin}\big(\omega_\text{d}(t-\tau)\big)
	\end{equation}
	...
	x(t) = 
	\begin{cases}
		\frac{0.2}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} t} \text{sin}(\omega_\text{d}t) & \text{if } t < \tau \\
		\frac{0.2}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} t} \text{sin}(\omega_\text{d}t)  + \frac{0.1}{m \omega_\text{d}} e^{-\zeta \omega_\text{n} (t-\tau)} \text{sin}\big(\omega_\text{d}(t-\tau)\big) & \text{if } \tau \leq t 
	\end{cases}
	\end{equation}
...
\begin{example}
	\textbf{Duhamel Integral for Base Excitation}

	\noindent Building on the impulse response formulation developed in Example~\ref{example:Double_Hammer_Impact}, we now extend this approach to arbitrary inputs using the Duhamel (convolution) integral, which represents the time-domain equivalent of the transfer function method.
	Consider the base exciton as shown below subjected to an arbitrary base excitation. Derive an equation (Duhamel integral) for its displacement $(z)$, when the displacement is expressed at the relative displacement of the mass such that $z = x - y$. 
	...
	Let $y(t)$ denote the prescribed base displacement. For simplicity, we will drop the explicit dependence on $t$ in what follows. The equation of motion for the system is then given by
	\begin{equation}
	...
	\end{equation} 	
	Given that we can replace $-m\ddot{y}$ with $F$ by applying Newton's second lat, this is the same equation as
	\begin{equation}
		m\ddot{z} + c \dot{z} + kz = F,
	\end{equation} 	
	meaning that the solutions for an arbitrary force-excited problem shown in Equation~\ref{eq:Duhamel_2} can transfer to a base-excited problem if we consider the relative displacement of the mass (here defined as $z$). Therefore, we can write the equation for the relative displacement of the mass as
	\begin{equation}
		z(t) = -\frac{1}{\omega_\text{d}} \int_{0}^{t} \ddot{y}(\tau)\, e^{-\zeta \omega_\text{n}(t-\tau)} \sin\!\big(\omega_\text{d}(t-\tau)\big)\, d\tau.
		\label{eq:Duhamel_base_excitation}
	\end{equation}	
Note that substituting $F(\tau) = -m\ddot{y}(\tau)$ into the Duhamel integral (Equation~\ref{eq:Duhamel_2}) results in cancellation of the mass term, which is why $m$ does not appear in Equation~\ref{eq:Duhamel_base_excitation} and the leading navigate sign is added.
\end{example}
...
\subsubsection{Definition of the Transfer Function $\mathbf{H(s)}$}
...
\begin{equation}
a_\text{n}\frac{d^nx(t)}{dt^n} + a_{n-1}\frac{d^{n-1}x(t)}{dt^{n-1}} + ... + a_0x(t) = b_m\frac{d^mf(t)}{dt^m} + b_{m-1}\frac{d^{m-1}f(t)}{dt^{m-1}} + ... + b_0f(t)
\end{equation} 
...
\begin{eqnarray}
&a_\text{n}s^nX(s) + a_{n-1}s^{n-1}X(s) + ... + a_0X(s) + \text{initial condition for } x(t) =   \\
...
\begin{eqnarray}
(a_\text{n}s^n + a_{n-1}s^{n-1} + ... + a_0)X(s) =  (b_ms^m + b_{m-1}s^{m-1} + ... + b_0)F(s) 
...
\begin{equation}
\frac{X(s)}{F(s)} = \frac{b_ms^m + b_{m-1}s^{m-1} + ... + b_0}{a_\text{n}s^n + a_{n-1}s^{n-1} + ... + a_0}
\end{equation}
...
\subsubsection{Frequency Response Function}
...
\begin{figure}[H]
	...
	\label{fig:1-DOF-spring_dashpot_mass_horizontal_forced-4.4.2}
\end{figure}
Considering the forced system shown in Figure~\ref{fig:1-DOF-spring_dashpot_mass_horizontal_forced-4.4.2} that can be expressed as the equation of motion
\begin{equation}
...
The frequency response function ($H(j\omega)$) of the system is the transfer function of the system evaluated in the complex plane.
...
&=  \sqrt{\frac{1}{(k-m\omega^2)^2+(c\omega)^2}} \\
...
\begin{review}
	...
	\begin{eqnarray}
	H(\omega) = |H(j\omega)| = \frac{1}{\sqrt{(k-m\omega^2)^2+(c\omega)^2}}
	\end{eqnarray}
\end{review}
			\vspace{-2ex}
\begin{example}
...
\subsubsection{Random Vibration Response}
...
\begin{note}
The autocorrelation function measures how similar a signal is to a time-shifted version of itself. For a sinusoidal input, the signal is highly structured and periodic.  As a result, the autocorrelation remains high and varies smoothly with the time shift $\tau$, as shown in Figure~\ref{fig:Response_to_random_input_autocorrelation}(a).

In contrast, a random (noise) signal has little to no predictable structure. Values at different times are largely independent, so shifting the signal over itself does not preserve similarity. Consequently, the autocorrelation drops to near zero for $\tau \neq 0$, indicating a lack of correlation between past and future values; shown in Figure~\ref{fig:Response_to_random_input_autocorrelation}(b).
 
In practical terms, autocorrelation reveals whether a vibration signal contains repeating patterns (e.g., harmonic motion) or is dominated by random fluctuations.
\end{note}
...
\textbf{Calculating Power Spectral Density}
...
\subsubsection{Statistical System Response Characterization}
...
\pagebreak			
			...
			\hspace*{1.5 in}\=\hspace{1.5in}\= \kill
				...
				$\dfrac{1}{\omega_\text{d}}e^{-\zeta \omega t}\sin(\omega_\text{d} t)$ \> $\dfrac{1}{s^2+2\zeta \omega s+\omega^2}$ \>\LTNUM \\ \\
				$1-\dfrac{\omega}{\omega_\text{d}}e^{-\zeta \omega t}\sin(\omega_\text{d} t + \phi)$, $\phi =\cos^{-1}(\zeta) \dots $ \\ \> $\dfrac{\omega^2}{s(s^2+2\zeta \omega s+\omega^2)}$ \>\LTNUM \\ \\
