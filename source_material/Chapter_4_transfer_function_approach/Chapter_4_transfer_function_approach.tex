...
In contrast, a random signal has no predictable structure. Values at different times are independent, so shifting the signal over itself does not preserve similarity. Consequently, the autocorrelation drops to near zero for $\tau \neq 0$, indicating a lack of correlation between past and future values; shown in Figure~\ref{fig:Response_to_random_input_autocorrelation}(b).
